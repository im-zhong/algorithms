...
enum rbtree_color { rbtree_red, rbtree_black };
...
static inline void rbtree_left_rotate_assign(rbtree_node* head,
                                             rbtree_node* parent,
                                             rbtree_node** child) {
    rbtree_node* right_child = parent->right;
    parent->right = right_child->left;
...
static inline void rbtree_right_rotate_assign(rbtree_node* head,
                                              rbtree_node* parent,
                                              rbtree_node** child) {
    rbtree_node* left_child = parent->left;
    parent->left = left_child->right;
        ...
        if (node->parent == node->parent->parent->left) {
            ...
            if (uncle->color == rbtree_red) {
                ...
                if (node == node->parent->right) {
                    node = node->parent;
                    // rbtree_left_rotate_assign(head, node,
                    // &node->parent->left);
        ...
        } else // node->parent == node->parent->parent->right
        {
            ...
            if (uncle->color == rbtree_red) {
                ...
                if (node == node->parent->left) {
                    node = node->parent;
                    // rbtree_right_rotate_assign(head, node,
                    // &node->parent->right);
...
static inline void rbtree_transplant(rbtree_node* head, rbtree_node* old_node,
                                     rbtree_node* new_node) {
    old_node == old_node->parent->left ? old_node->parent->left = new_node
                                       : old_node->parent->right = new_node;
    new_node->parent = old_node->parent;
}

static inline void rbtree_transplant_assign(rbtree_node* head,
        ...
        if (node == node->parent->left) {
            ...
            if (sibling->left->color == rbtree_black &&
                ...
                if (sibling->left->color == rbtree_red) {
                    ...
                    sibling->color = rbtree_red;
                    // rbtree_right_rotate_assign(head, sibling,
                    // &node->parent->right);
            ...
            if (node == node->parent->right) {
                ...
                if (sibling->left->color == rbtree_black &&
                    ...
                    if (sibling->right->color == rbtree_red) {
                        sibling->right->color = rbtree_black;
                        sibling->color = rbtree_red;
                        // rbtree_left_rotate_assign(head, sibling,
                        // &node->parent->left);
...
void rbtree_erase_impl(rbtree_node* head, rbtree_node* node) {
    ...
    if (node->left == head) {
        ...
        if (right_first->parent == node) {
            ...
            // rbtree_transplant_assign(head, right_first, right_first->right,
            // &right_first->parent->left);
...
bool is_rbtree_impl(int& last_black_height, int current_black_height,
                    rbtree_node* head, rbtree_node* root) {
    if (root == head)
        return true;

    // 不能有两个连续的黑色节点
    if (root->color == rbtree_red) {
        if (root->left->color == rbtree_red ||
            root->right->color == rbtree_red) {
    ...
    if (is_leaf(head, root)) {
        if (last_black_height == 0) {
            ...
            if (last_black_height == current_black_height) {
                // std::cout << "current black height: " << current_black_height
                // << std::endl; std::cout << "last black height: " <<
                // last_black_height << std::endl; 正常
            } else {
                std::cout << "different black height at " << root->value
                          << "\n";
                std::cout << "current black height: " << current_black_height
                          << std::endl;
                std::cout << "last black height: " << last_black_height
                          << std::endl;
                return false;
            }
        }
        ...
        if (!is_rbtree_impl(last_black_height, current_black_height, head,
                            root->left))
            return false;
        if (!is_rbtree_impl(last_black_height, current_black_height, head,
                            root->right))
...
void performance(int count) {
    ...
    std::cout << "rbtree is faster than set by "
              << static_cast<double>(set_time) /
                     static_cast<double>(rbtree_time)
              << "\n";
