...
// TODO:
// 完成一个交互式的树结构CLI，支持插入删除等操作，并且需要实时更新绘制出来的图片来展示树的结构
...
void binary_insert(int value, rbtree_node *T) {
  ...
  while (work->element != value) {
    ...
    // 在一开始的时候，T是head
    // 而work = parent = T = head
    // 循环开始判断 head->element == value,
    // 因为head->element是负无穷，自然不相等 进入循环体，parent = work = head
...
enum DeleteFlag { NORMAL, FIND_LEFT_MAX, FIND_RIGHT_MIN };
...
rbtree_node *rbtree_rotate(rbtree_node *grand_parent, rbtree_node *parent,
                           rbtree_node *child) {
...
void rbtree_rotate_assign_2(rbtree_node *grand_grand_parent,
                            rbtree_node *grand_parent, rbtree_node *parent,
                            rbtree_node *child) {
  // 这里是两种实现方法
  // 如果用指针的指针，这里可以如何写??
...
bool has_red_left_child(rbtree_node *node) { return node->left->color == kRed; }
...
static inline void rbtree_replace_child(rbtree_node *parent,
                                        rbtree_node *old_child,
                                        rbtree_node *new_child) {
  old_child == parent->left ? parent->left = new_child
...
// 我们可以知道的是 P 和
// X的相对位置关系，因为带调整颜色的代码前面需要向下搜索X，此时可以知道X与P的相对位置关系
// 	 P
// X  sib
...
void do_gen_dot(rbtree root, std::string &dot, bool is_color) {
  if (root != nullnode) {
    if (is_color) {
      if (root->color == kRed) {
        dot.append("  " + std::to_string(root->element) +
                   " [fontcolor = red]\n");
      }
    }
    if (root->left != nullnode) {
      dot.append("  " + std::to_string(root->element) + " -> " +
                 std::to_string(root->left->element) + "\n");
    }
    if (root->right != nullnode) {
      dot.append("  " + std::to_string(root->element) + " -> " +
                 std::to_string(root->right->element) + "\n");
    }
  ...
  if (is_leaf(root)) {
    if (last_black_height == 0) {
      ...
      if (last_black_height == current_black_height) {
        ...
        // std::endl; std::cout << "last black height: " << last_black_height <<
        // std::endl; 正常
      } else {
...
void performance(int count) {
  ...
  std::cout << "rbtree is faster than set by "
            << static_cast<double>(set_time) / static_cast<double>(rbtree_time)
            << "\n";
