...
static inline void hash_table_insert(hash_table *table, list_entry *entry) {
  // 因为这个
  hash_table_insert_impl(table, hash_table_index_slot(table, entry->key),
                         &entry->node);
...
static inline void hash_table_insert_impl(hash_table *table, list_node_t *head,
                                          list_node_t *node) {
...
static inline void hash_table_move(hash_table *old_table,
