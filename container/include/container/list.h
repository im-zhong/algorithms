...
static inline void list_replace(list_node_t *old, list_node_t *new_node) {
...
static inline bool list_is_first(const list_node_t *head,
                                 const list_node_t *node) {
  return node == list_first(head);
}

static inline bool list_is_last(const list_node_t *head,
                                const list_node_t *node) {
...
// 把链表从node处分开
// static inline void list_cut(list_node_t *head, list_node_t *node, list_node_t
// *list)
// {
//     list->next = node->next;
//     list->next->prev = list;
...
// list: new_node list head
static inline void list_cut_after(list_node_t *head, list_node_t *node,
                                  list_node_t *list) {
  list->next = node->next;
  list->next->prev = list;
  list->prev = head->prev;
...
static inline void list_cut_before(list_node_t *head, list_node_t *node,
                                   list_node_t *list) {
  list->next = node;
  list->prev = head->prev;
  list->prev->next = list;
...
static inline void list_insert_bulk_between(list_node_t *prev,
                                            list_node_t *next,
                                            list_node_t *first,
                                            list_node_t *last) {
  prev->next = first;
...
static inline void list_splice_after(struct list_node_t *head,
  ...
  list_insert_bulk_between(head, head->next, list_first(list), list_last(list));
}

static inline void list_splice_before(struct list_node_t *head,
                                      const struct list_node_t *list) {
  list_insert_bulk_between(head->prev, head, list_first(list), list_last(list));
}
