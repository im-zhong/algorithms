...
static inline vertex_entry *make_vertex_entry(vertex_t vertex,
                                              weight_t weight) {
...
void bfs(spares_graph *graph, vertex_t vertex) {
  ...
  while (!queue_is_empty(&queue)) {
    ...
    list_for_each_entry(adjacency_entry, &(graph->vertex_array[vertex]),
                        vertex_entry, queue_) {
      // 判断，当前节点是否被遍历过了
      if (vertex_property[adjacency_entry->vertex].color == white) {
        ...
        queue_push(&queue, make_vertex_entry(adjacency_entry->vertex,
...
void dfs_visit(spares_graph *graph, graph_dfs *df_forest, vertex_t vertex,
               size_t time) {
  ++time;
...
static inline void relax(spares_graph *graph, sptree_t *sptree, vertex_t u,
